...
如果我们对某一层开启 **block-wise gradient checkpointing**：
...
> - 如果我们把 B=2、S=2048，激活变 ~24GB，总显存就接近 80GB 边缘了。
...
> ✅ 这就是一个非常现实、而且在工程上广泛使用的组合：**「bf16 权重 + 8bit Adam 优化器 + 梯度检查点」在单卡 80GB 上全参微调 8B 级模型**，既省显存又相对容易实现，还给我们留足了 20GB+ 的缓冲空间。
...
> 如果我们想要最保守、永远不低估的上界，也可以用 **16 B/param**（相当于“有 FP32 主权重 + FP32 m/v”的老方案），那对应 8B 模型就是我们一开始算的 ~122GB。
...
- **Act(B,S)**：
  - 看我们是否开梯度检查点，选择 $\text{Act}_{\text{no-ckpt}}$ 或 $\text{Act}_{\text{ckpt}}$ 公式；
  - 代入实际的 batch $B$ 和 seq $S$ 计算。
- **Overhead**：
  - PyTorch 内部缓冲区、临时张量、损失 / logit 等，一般预留 **1–3GB** 就够了。

> 有了这个速算模板，我们换模型（比如从 8B → 14B）、换精度（Case C → D）、换 batch/seq 的时候，只要替换 Base 或 $B,S$，就能快速估一个「能不能塞进这张卡」的数量级。
...
如果我们把这些需要在 backward 时用到的向量都算上，一个 token 在一层里的注意力部分，**大概会有若干个 “d 维向量” 需要保留**：
...
- **残差分流需要边界激活：**
  - 在 $x_{l+1}=h_1+b$ 处，必须把 $g_{x_{l+1}}$ **一份直接给 $h_1$**，另一份走 $b$（MLP）链路；这要求我们在该边界拥有 **$h_1$**（或 $x_{l+1}$ 之一）来“接”住梯度并继续传播。
