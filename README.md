...
_作者注：本项目与AI共同开发完成，部分文档和代码由AI撰写_
...
3. **前沿性**：涵盖 GRPO、DAPO、CPPO 等 2024 年最新算法
...
| **Day 5** | LLM-RL | 实战落地 | RLHF, 奖励模型, GRPO, DAPO |
...
```
    ...
    │   SFT   │    →     │   DPO   │ →  │ RL 理论  │ → │ LLM-RL  │ →  │   CL    │ → │ LLM-CL  │
    │ 监督微调 │           │ 偏好优化 │    │ 基础理论 │   │ 实战应用 │     │ 经典方法 │   │ 前沿算法 │
    └─────────┘          └─────────┘    └─────────┘   └─────────┘    └─────────┘   └─────────┘
        ↓                     ↓              ↓            ↓               ↓             ↓
    "模仿专家"              "区分好坏"      "理解原理"     "获得奖励"       "防止遗忘"     "持续进化"
```
...
- Qwen3-1.7B/**Qwen3-4B** / **Qwen3-8B**：适合单卡训练
