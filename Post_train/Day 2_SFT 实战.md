...
**assistant-only loss mask（仅助手侧损失）**
 SFT 本质是因果语言建模，但只对“助手角色（assistant）”产生的 token 计损失；对 user/system（以及 prompt 部分）不计损失。这样可以避免模型去“背诵”用户输入，强化“从指令到回复”的映射。实现上通常把非 assistant 段的 `labels` 置 `-100`（即忽略梯度）即可。你的计划文档也明确了 Day2 的这个要点：SFT 的本质=因果 LM + assistant-only loss mask，并说明“不对 user/system 计损失（避免复述）”  。

**QLoRA（4-bit 量化 + LoRA 低秩适配）**
 核心是：把**基座模型权重**做 4-bit（nf4）量化并冻结，只训练插在某些线性层上的 **LoRA 低秩矩阵**（rank=r），再配合缩放系数（α）和 LoRA dropout 控制容量/正则。先量化，再lora微调。优点：显著降显存（单卡 16 GB 就能训 7B/8B），训练稳定、可拔插回退；你的计划文档强调了“LoRA 低秩近似，r/α/Dropout 的作用；QLoRA 的 4-bit nf4 量化与误差补偿”  。在 Day1 里也给了 4080 友好的基座建议：Qwen2.5-7B-Instruct 或 Llama-3.1-8B-Instruct + QLoRA  。
...
> 多轮训练通常**只对最后一个 assistant 段计损失**（或对每个回合的 assistant 都计损失，但要确保模板能逐段正确打 mask）。Qwen3 的模板在“多个 assistant 消息”情况下发生过小问题的修复讨论，你的数据与评测也应覆盖这类样例以防模板边界错配。[Hugging Face](https://huggingface.co/Qwen/Qwen3-1.7B/discussions/9)
...
Qwen3 在 `apply_chat_template` 时支持 `enable_thinking=True/False`（是否插入“思考 token/段”）；模型卡与讨论显示了这个开关的用法。**数据集层面**你可以准备两类样本：
...
> 训练时是否真的需要“思考段”由你决定——Qwen3 的模板**不强制**思考。[Hugging Face](https://huggingface.co/blog/qwen-3-chat-template-deep-dive)
...
**不要手工写**。你在构造数据集时，应该保持干净的 JSON 格式：
...
1. **模板稳定**：不同版本的 Qwen 可能更新模板，如果你手工写符号，将来不兼容。
...
> 所以 `<|im_start|>` `<|im_end|>` 这类特殊符号只会在 **最终训练文本里**出现，由 tokenizer 模板自动插入；你自己准备的数据集 JSON 不需要也不应该直接写它们。
...
具体做法，请见`build_datasets.py`。同时也可以使用`check_dataset.py` 来验证生成的数据集是否符合qwen3训练的规范。当然，HF也提供了官方的数据处理方法，如果你不想自己处理，也可以使用**Hugging Face Datasets**：
...
想象一下，你是一个小孩子，正在学画画。你的老师给你一些画笔、颜料和纸张，然后你开始画一个苹果🍎。

现在，假设你有一个机器人朋友，它也想学画画。但是，它不知道怎么开始。于是，你的机器人朋友会观察你画画的过程，记住你的动作和步骤，然后模仿你画苹果的样子。
...
```
你好呀！✨ 我是Qwen，是阿里云研发的超大规模语言模型。你可以叫我通义千问，或者直接叫Qwen。我就像一个知识渊博的助手，能帮你解答各种问题，无论是写故事、写公文、写邮件、写剧本，还是做数学题、编程、逻辑推理，甚至可以陪你聊天、玩游戏，我都能尽力提供帮助。

我支持多种语言，包括中文、英文、德语、法语、西班牙语、葡萄牙语、俄语、阿拉伯语、日语、韩语等，可以和你用你最熟悉的语言交流。

无论你是学生、老师、程序员、创业者，还是只是想找个聊天伙伴，我都很乐意陪你一起探索世界、解决问题、激发灵感！
...
```
我靠！😊 你好呀！我是通义千问，中文智者！ 🤖️ 我是一个超智能的语言模型，能够回答各种问题、提供帮助和陪伴你聊天。 💬️ 我可以写诗、写小说、写文章、写代码等等！ 📝️ 我还能帮你做数学题、科学实验、编程设计等等！ 🔍️ 所以，叫我通义千问吧！ 👋️
```
...
- 如果你用 24GB 显存的卡，开 `per_device_train_batch_size=4`，意味着每次梯度计算就处理 4 条样本。
...
3. **缺少混合训练**
   - 工业界常见做法是：
     ...
     - 再混合你的任务数据一起训练（称为 **multi-task SFT**）。
   - 如果只喂进单语数据，等于是强行“单语迁移”，模型会失去泛化。
4. **步数少但影响大**
   - 即便你只训了几十/几百步，学习率高（2e-4）+ LoRA 更新，也足够把模型分布往单语/单任务推走。
