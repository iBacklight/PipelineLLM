...
我们在之前介绍DPO和PPO的时候已经详细阐述过KL散度。在这里，KL也是LLM-RL中最独特也最重要的组成部分之一，它告诉模型：“我们可以优化奖励，但不能丢掉我们最开始学到的、流畅自然的说话方式。”
...
- **直观理解**：**KL惩罚是“不忘初心”的约束。** 它告诉模型：“我们可以优化奖励，但不能丢掉我们最开始学到的、流畅自然的说话方式。”
...
可能我们也注意到了，BT模型本质上是一个二元的偏好模型（Pairwise Model），如何去建模多K个回答呢？原文中提到，他们虽然给标注员看K候选做排序，但训练时把这一组排序**拆成所有成对比较**（共有 $\binom{K}{2}$ 对），并用**对比式 logistic 损失**来拟合奖励模型分数差 $r_\theta(x,y_w)-r_\theta(x,y_l)$。他们还强调把同一提示的所有成对比较作为**一个 batch 元素**训练，以避免把所有对打散后出现的过拟合问题。
...
\;}
$$
我们可能也发现了，这不就是DPO的损失函数吗？没错，**DPO确实借鉴了RM训练的核心思想**，但它做了一个根本性的改变，从而绕过了整个复杂的RL流程。具体来说，这里的$f_\phi(x,y)$依然是依赖于人类评判，而DPO则将$f_\phi(x,y)$直接换成了$\pi_{new}$和$\pi_{old}$的比较。
...
所以我们会在基座的QWEN模型后加一个由linear层组成的rewards head，当然我们也可以自定义它的形状
...
pass@k 表示，如果我们为每个测试用例生成许多候选输出（或从模型生成多次），那么从中随机选 k 个，至少有一个是“正确”的概率。它常用来评估代码生成/数学题等“有明确定义的正确性判定”的任务。pass@1 即 k=1。

设对某题我们总共采样了 $n$ 个输出，其中 $c$ 个是正确的（$0\le c\le n$）。从这 $n$ 个输出**不放回地**随机抽取 $k$ 个，抽到**至少一个正确**的概率为：
$$
